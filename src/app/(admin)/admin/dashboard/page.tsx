...
import { adminService, type AdminDashboardOverview } from "@/services/admin.service";
...
export default function AdminDashboardPage() {
  ...
  if (error) {
    return (
      <Card className="border-destructive">
        <CardBody>
          <p className="text-destructive font-medium">Failed to load dashboard: {error}</p>
          <p className="text-sm text-muted-foreground mt-2">
            Ensure backend is running and you are logged in as Admin.
          </p>
        </CardBody>
      </Card>
    );
  ...
  const revenueFormatted = new Intl.NumberFormat("en-TZ", {
    style: "decimal",
  ...
  const metrics = [
    ...
    {
      label: "Response Rate",
      value: `${data.responseRatePercent.toFixed(1)}%`,
      icon: BarChart3,
      color: "text-primary-dark",
      bg: "bg-primary-dark/10",
      trend: "-1%",
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {metrics.map(({ label, value, icon: Icon, color, bg, trend, up }) => (
          <Card
            key={label}
            className="group border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 rounded-2xl hover:-translate-y-0.5"
          >
            <CardBody className="flex flex-col gap-3 p-5">
              ...
              <div className="mt-2">
                <p className="text-muted-foreground text-[14px] font-medium">{label}</p>
                <p className="text-2xl font-bold text-foreground mt-1 tracking-tight break-all">
      ...
      <Card className="border border-border bg-card rounded-2xl">
        ...
        <CardBody className="px-6 pb-6 flex flex-wrap gap-3">
          <Button as={Link} href="/admin/moderation" color="primary" className="rounded-xl">
            Review pending jobs
          </Button>
          <Button as={Link} href="/admin/users" variant="bordered" className="rounded-xl border-primary text-primary hover:bg-primary/10">
            Manage users
          </Button>
          <Button as={Link} href="/admin/categories" variant="bordered" className="rounded-xl border-primary text-primary hover:bg-primary/10">
            Edit categories
          </Button>
          <Button as={Link} href="/admin/disputes" variant="flat" className="rounded-xl text-muted-foreground hover:bg-primary/10">
            View disputes
          </Button>
        </CardBody>
      </Card>
