...
const dummyMessages = [
    { id: "1", avatar: "K", avatarColor: "#006e42", sender: "Kuna", preview: "Hi, I'm interested in your job post...", time: "5m ago", unread: true },
    { id: "2", avatar: "J", avatarColor: "#1a73e8", sender: "Jared B. Watson", preview: "Thanks for the interview opportunity", time: "1h ago", unread: true },
    { id: "3", avatar: "M", avatarColor: "#f59e0b", sender: "Matting", preview: "When can we schedule a call?", time: "3h ago", unread: false },
];

const dummyNotifications = [
    { id: "1", icon: "✅", title: "Application Received", message: "Kuna applied to your job: AI content writer", time: "10m ago", unread: true },
    { id: "2", icon: "📋", title: "Job Posted", message: "Your job 'OnlyFans Chatter' is now live", time: "1h ago", unread: true },
    { id: "3", icon: "💬", title: "New Message", message: "Jared B. Watson sent you a message", time: "2h ago", unread: false },
    { id: "4", icon: "🔔", title: "Reminder", message: "Complete your company profile to attract more workers", time: "1d ago", unread: false },
];
...
export function ClientTopbar() {
    ...
    useEffect(() => {
        if (showNotifications && !notifsUseSample) {
            notificationService.list({ size: 20 })
                .then((p) => setNotifications(p?.content ?? []))
                .catch(() => setNotifsUseSample(true));
        }
    }, [showNotifications, notifsUseSample]);
                        ...
                        <div className="absolute top-[calc(100%+10px)] right-0 w-[360px] bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.12)] border border-[#f0f0f0] z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                            ...
                            <div className="overflow-y-auto max-h-[340px]">
                                {dummyMessages.map((msg) => (
                                    <div
                                        key={msg.id}
                                        className={`flex items-start gap-3 px-5 py-3.5 hover:bg-[#f9fafb] transition-colors cursor-pointer border-b border-[#f9f9f9] last:border-0 ${msg.unread ? "bg-[#f0faf5]" : ""}`}
                                    >
                                        <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-[14px] font-bold shrink-0" style={{ backgroundColor: msg.avatarColor }}>
                                            {msg.avatar}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between gap-2">
                                                <span className="text-[13.5px] font-bold text-[#111827] truncate">{msg.sender}</span>
                                                <span className="text-[11px] text-[#999] shrink-0">{msg.time}</span>
                                            </div>
                                            <p className="text-[12.5px] text-[#666] truncate mt-0.5">{msg.preview}</p>
                                        </div>
                                        {msg.unread && <div className="w-2 h-2 rounded-full bg-[#006e42] mt-2 shrink-0" />}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="absolute top-[calc(100%+10px)] right-0 w-[380px] bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.12)] border border-[#f0f0f0] z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                            ...
                            <div className="overflow-y-auto max-h-[360px]">
                                {(notifsUseSample ? dummyNotifications : displayNotifs).map((notif) => {
                                    ...
                                    return (
                                        <div
                                            key={(notif as NotificationItem).id ?? (notif as { id: string }).id}
                                        ...
                                        >
                                            <div className="w-10 h-10 rounded-full bg-[#f0faf5] flex items-center justify-center text-[20px] shrink-0">{icon}</div>
                                            <div className="flex-1 min-w-0">
